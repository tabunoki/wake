...
バインド変数はテンプレートの内容をコンテンツから決定する一種の変数です。
...
`<title>{{wake}}</title>` になっているということです。
`{{wake}}` というバインド変数を記述することによって、
その定義をコンテンツファイルに委ねることが出来ます。


### テンプレート側の定義

バインド変数を記述するには以下の正規表現にマッチする必要があります。
...
例えば、コンテンツが先ほどのバインド変数を利用する場合、
`{{title}}` の定義を委ねられているため、その内容をコンテンツで定義する必要があります。

	@title = Tutorial

`@` の後に続いて、バインド変数名を記述します。
`=` の後に続いて、バインド変数の値を記述します。
`=` の前後のスペースは無視されます。

これによって、テンプレートの `{{title}}` という記述が、_Tutorial_ に置き換わります。
